...
project_path=$(pwd)
#apk文件夹路径
ios_path=${project_path}/ipa/$env
pay_load=Payload
runner_app=Runner.app

#Flutter打包生成的最初地址 build/app/outputs/apk
release_path=${project_path}/build/ios/archive/Runner.xcarchive/Products/Applications/Runner.app
...
if (($((ch_num)) == 0)); then
  ...
  for ((i = 0; i < $((channel_len)); i++)); do
    ...
    cd $project_path
    flutter build ipa --dart-define=PACKAGE_ENV="${env}" --dart-define=CHANNEL="${channel}"   --release
    cp -R "${release_path}" "${ios_path}"
    cd $ios_path
    mkdir $pay_load
    cp -R $runner_app $pay_load 
    zip -r "cfox_${env}_${channel}.ipa" "${pay_load}"
...
# 判断apk目录下是否有文件
if [ "$(ls -A "${ios_path}")" ]; then
  echo "================ APK包已导出:${ios_path} 【耗时：${var_time}s】==============="
  open "${ios_path}"
else
